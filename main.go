...
import (
	...
	"twitter/conf"
	"twitter/db"
	migrate "twitter/db/migration"
	"twitter/handlers"
	"twitter/routes"
	"twitter/utils/secure"
)
	...
	logHandler := slog.NewTextHandler(os.Stdout, &slog.HandlerOptions{
		...
		ReplaceAttr: func(groups []string, a slog.Attr) slog.Attr {
			if a.Key == slog.TimeKey {
				...
				lastIndex := strings.LastIndex(filepath, "twitter-miniapp-backend/")
				if lastIndex != -1 {
					msg := filepath[lastIndex+1+len("twitter-miniapp-backend"):] + ":" + fileline[:len(fileline)-1]
